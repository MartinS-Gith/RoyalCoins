const LOCAL_CONFIG_SELL = {
    //Precio base en BRL por coin (Compra)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sellerCharName) {
        sellerCharName.setAttribute('autocomplete', 'off');
        sellerCharName.addEventListener('input', () => {
             ...
             const name = sellerCharName.value.trim();
             generateVerifyLink(name, sellerCharName);
             if(name.length > 0) sellerCharName.classList.remove('is-invalid');
        });
    }
...
function calculatePayout() {
    ...
    if (amount && amount >= 250) { 
        const valueInBrl = amount * LOCAL_CONFIG_SELL.baseBuyPricePerCoinBRL;
        const rate = exchangeRates[currentCurrency] || 1;
        let finalValue = valueInBrl * rate;
        if (['CLP', 'ARS'].includes(currentCurrency)) finalValue = Math.round(finalValue);
...
function generateWhatsAppLink() {
    ...
    if (!checkTerms.checked) {
        sellWarning.innerText = "⚠️ Acepta términos."; 
        sellWarning.style.display = 'block'; return;
    }

    if (charName.length === 0) {
        sellWarning.innerText = "❌ Falta nombre del personaje."; 
    ...
    if (!email || !email.includes('@')) {
        ...
        errorMsg = "❌ Debes escribir el nombre del titular.";
    }
    ...
    const msg = `💎 *SOLICITUD DE VENTA (RUBINI COINS)*\n` +
                `────────────────────────\n` +
                `📤 *Entrego:* ${amount} RC\n` +
                `💵 *Recibo:* ${totalPayout}\n` +
                `👤 *Personaje:* ${charName}\n` +
    ...
    if(currentCurrency === 'CLP' && inputAccountType && inputAccountType.value) modalBankInfo += `\n${inputAccountType.value}`;
    ...
    bankDetailsContainer.innerHTML = `
        <div class="d-flex justify-content-between align-items-start">
            ...
            <button class="btn btn-sm btn-outline-primary ms-2 btn-copy" title="Copiar"><i class="bi bi-clipboard"></i></button>
        </div>`;
